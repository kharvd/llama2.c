...
def encode(str, bos, eos):
    tokens = [BOS_TOKEN] if bos else []
    tokens.extend([int(c) for c in str])
    tokens.append(EOS_TOKEN) if eos else []
    return tokens


def pretokenize():
    data_file = os.path.join(DATA_CACHE_DIR, "primes_shuffled.txt")

    all_tokens = []
    with open(data_file, "r") as f:
        for line in tqdm(f):
            text = line.strip()
            tokens = encode(text, bos=True, eos=True)
...
class PretokDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        data_file = os.path.join(DATA_CACHE_DIR, "primes_shuffled.bin")
