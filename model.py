...
class TransformerBlock(nn.Module):
    ...
    def collect_activations(self, x, freqs_cos, freqs_sin):
        h = x + self.attention.forward(self.attention_norm(x), freqs_cos, freqs_sin)
...
class Transformer(nn.Module):
    ...
    def collect_activations(self, tokens: torch.Tensor, layer_idx: int) -> torch.Tensor:
        _bsz, seqlen = tokens.shape
        h = self.tok_embeddings(tokens)
        h = self.dropout(h)
        freqs_cos = self.freqs_cos[:seqlen]
        ...
        h = self.layers[layer_idx].collect_activations(h, freqs_cos, freqs_sin)

        return h.view(_bsz * seqlen, -1)
